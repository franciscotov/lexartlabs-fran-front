...
export default function PixelArtGreetingCard() {
  ...
  const [tool, setTool] = useState<'pen' | 'eraser' | 'fill' | 'text' | 'particle'>('pen');
  ...
  useEffect(() => {
    if (isPlaying && frames.length > 1) {
      ...
      const interval = setInterval(animate, playbackSpeed);
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-4 mb-4">
          ...
          <div className="flex justify-around mt-4">
            <button
              onClick={() => setTool('pen')}
              className={`p-3 rounded-lg ${tool === 'pen' ? 'bg-purple-500 text-white' : 'bg-gray-100'}`}
            >
                Square
              {/* <Square size={20} /> */}
            </button>
            <button
              ...
              className={`p-3 rounded-lg ${tool === 'eraser' ? 'bg-purple-500 text-white' : 'bg-gray-100'}`}
            >
                Circle
              {/* <Circle size={20} /> */}
            </button>
            <button
              ...
              className={`p-3 rounded-lg ${tool === 'text' ? 'bg-purple-500 text-white' : 'bg-gray-100'}`}
            >
                Type
              {/* <Type size={20} /> */}
            </button>
            <button
              onClick={() => setTool('particle')}
              className={`p-3 rounded-lg ${tool === 'particle' ? 'bg-purple-500 text-white' : 'bg-gray-100'}`}
            >
                Sparkles
              {/* <Sparkles size={20} /> */}
            </button>
          ...
          <div className="grid grid-cols-6 gap-2 mt-4">
            {COLORS.map(color => (
              <button
                ...
                className={`h-10 rounded-lg border-2 ${selectedColor === color ? 'border-purple-500' : 'border-gray-300'}`}
